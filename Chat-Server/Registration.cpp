...
std::pair<std::string, bool> Registration::handleRegistration(const char registrationString[], Database& database)
{
	std::vector<std::string> registrationParts;
	boost::split(registrationParts, registrationString, boost::is_any_of(" "));
	if (registrationParts.size() != 3)
	{
	...
	auto users =
		database.execute("SELECT username FROM users WHERE username = ?", {registrationParts.at(1)});
	if (users.size() != 0)
	{
		return {registrationParts.at(1), false}; // username already exists, abort!
	}

	// std::cout << "Your username is: " << registrationParts.at(1) << " and password is: " <<
	// registrationParts.at(2) << "\n";

	database.execute("INSERT INTO users(username, password) VALUES(?, ?)",
		{registrationParts.at(1), registrationParts.at(2)});
