...
Session::Session(tcp::socket socket, Dispatcher& other_dispatcher, std::string username)
	: User(move(username)), dispatcher(other_dispatcher), session_socket(std::move(socket))
{
}
...
void Session::do_read()
{
	...
	session_socket.async_read_some(
		read_msg.mutable_buffer(), [this, self](boost::system::error_code ec, size_t length) {
			if (!ec)
			{
...
void Session::do_write()
{
	...
	boost::asio::async_write(session_socket, buf, [this, self, buf](boost::system::error_code ec, size_t length) {
		if (!ec && length == boost::asio::buffer_size(buf))
		{
			// mark message as delivered
			dispatcher.get_db().execute(
				"UPDATE messages SET delivered = 1 WHERE id = ?", {msg_queue.front().second});
